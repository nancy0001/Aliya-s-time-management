...
const STORAGE_KEY = "aliya-time-manager-v1";
const PLAN_KEY = "aliya-time-manager-plan-v2";
const LEGACY_PLAN_KEY = "aliya-time-manager-plan-v1";
...
function activeSopDateByClock(now = new Date()) {
...
function defaultGoalTargets(): GoalTarget[] {
  return [
    { id: uid(), cycle: "日度", taskName: "日度目标", priority: "P1", targetDesc: "核心任务时间块优先", note: "建议先完成高价值任务" },
    { id: uid(), cycle: "周度", taskName: "周度目标", priority: "P1", targetDesc: "深度工作优先配额", note: "周中检查偏差并修正" },
    { id: uid(), cycle: "月度", taskName: "月度目标", priority: "P2", targetDesc: "复利型任务持续投入", note: "月末复盘结构与投入比例" }
  ];
}
  ...
  if (raw) {
    try {
      ...
      let next: PeriodPlanState = {
        ...defaultPlans(),
        ...parsed,
        goalTargets: parsed.goalTargets && parsed.goalTargets.length ? parsed.goalTargets : defaultGoalTargets(),
...
function shiftDateBy(base: string, delta: number) {
  const d = parseLocalDate(base);
  d.setDate(d.getDate() + delta);
  return formatLocalDate(d);
}
function sumMinutes(list: TimeEntry[]) {
...
function biWeekStartOf(dateStr: string) {
...
function monthDates(baseDate: string) {
...
function buildMonthCells(baseDate: string) {
  const d = parseLocalDate(baseDate);
...
function aggregateByCategory(items: TimeEntry[]) {
...
function donutSlices(categoryMap: Record<TimeCategory, number>, cx = 100, cy = 100, r = 70) {
...
export default function TimeManagerApp() {
  ...
  const goalAdd = () => {
    setPlans((prev) => ({
      ...
      goalTargets: [
        ...prev.goalTargets,
        { id: uid(), taskName: "新增目标", cycle: "周度", priority: "P2", targetDesc: "", note: "" }
      ]
    }));
  };
  const updateGoal = (id: string, patch: Partial<GoalTarget>) => {
    setPlans((prev) => ({ ...prev, goalTargets: prev.goalTargets.map((g) => (g.id === id ? { ...g, ...patch } : g)) }));
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      const payload = JSON.parse(json) as TimeManagerSyncPayload;
      if (!Array.isArray(payload.entries) || !payload.plans) {
  ...
  const exportSyncCode = async () => {
    const payload: TimeManagerSyncPayload = {
  ...
  const importSyncCode = () => {
    ...
    try {
      ...
      const payload = JSON.parse(json) as TimeManagerSyncPayload;
      if (!Array.isArray(payload.entries) || !payload.plans) throw new Error("invalid payload");
      setEntries(payload.entries);
      saveEntries(payload.entries);
      setPlans(payload.plans);
      savePlans(payload.plans);
  ...
  return (
    <main className="app-shell morandi-time">
      ...
      <section className="card" style={{ marginTop: "0.55rem", marginBottom: "0.65rem" }}>
        <strong>跨页面数据同步</strong>
        <p className="muted-copy">当 Codex 页面和网页版不在同一地址时，用这里同步当前数据。</p>
        <div style={{ display: "flex", gap: "0.45rem", flexWrap: "wrap" }}>
          <button type="button" className="entry-toggle" onClick={exportSyncCode}>导出同步码</button>
          <button type="button" className="entry-toggle" onClick={importSyncCode}>导入同步码</button>
          <button type="button" className="entry-toggle" onClick={copySyncLink}>复制自动导入链接</button>
        </div>
        {syncMessage ? <p className="muted-copy" style={{ marginTop: "0.45rem" }}>{syncMessage}</p> : null}
      </section>
      <header className="hero"><div><p className="eyebrow">Aliya Time Manager</p><h1>时间管理系统</h1><p className="hero-copy">记录每天时间投入 做双日与周度复盘 看清时间都花在哪里 版本 {VERSION}</p></div></header>
      ...
      <section className="section">
        <div className="section-heading"><h2>周期目标管理</h2></div>
        <div className="card">
          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "0.45rem" }}>
            <button type="button" className="entry-toggle" onClick={goalAdd}>新增目标</button>
          </div>
          <ul className="log-list">
            {plans.goalTargets.map((g, idx) => (
              <li key={g.id} onClick={() => setEditingGoalId(g.id)}>
                {editingGoalId === g.id ? (
                  <form className="form-grid" onSubmit={(e) => { e.preventDefault(); setEditingGoalId(null); }}>
                    <label><span>任务名</span><input value={g.taskName} onChange={(e) => updateGoal(g.id, { taskName: e.target.value })} /></label>
                    <label><span>目标周期</span><select value={g.cycle} onChange={(e) => updateGoal(g.id, { cycle: e.target.value as GoalCycle })}><option value="日度">日度</option><option value="周度">周度</option><option value="月度">月度</option></select></label>
                    <label><span>优先级</span><select value={g.priority} onChange={(e) => updateGoal(g.id, { priority: e.target.value as GoalPriority })}><option value="P0">P0</option><option value="P1">P1</option><option value="P2">P2</option><option value="P3">P3</option><option value="P4">P4</option><option value="P5">P5</option></select></label>
                    <label><span>目标描述</span><textarea rows={2} value={g.targetDesc} onChange={(e) => updateGoal(g.id, { targetDesc: e.target.value })} /></label>
                    <label><span>说明</span><textarea rows={2} value={g.note} onChange={(e) => updateGoal(g.id, { note: e.target.value })} /></label>
                    <button type="submit">保存目标</button>
                  </form>
                ) : (
                  <>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.35rem 0.9rem" }}>
                      <div style={{ gridColumn: "1 / -1", display: "flex", justifyContent: "space-between", alignItems: "center" }}><strong>{g.taskName}</strong><span className="detail-value">{g.cycle}周期</span></div>
                      <span className="detail-line" style={{ gridColumn: "1 / -1" }}><span className="detail-key">目标描述</span><span className="detail-value">{g.targetDesc || "-"}</span></span>
                      <span className="detail-line"><span className="detail-key">优先级</span><span className="detail-value">{g.priority}</span></span>
                      <span className="detail-line"><span className="detail-key">说明</span><span className="detail-value">{g.note || "-"}</span></span>
                    </div>
                    <div style={{ display: "flex", gap: "0.35rem", marginTop: "0.45rem" }} onClick={(e) => e.stopPropagation()}>
                      <button type="button" className="entry-toggle" onClick={() => goalMove(g.id, -1)} disabled={idx === 0}>上移</button>
                      <button type="button" className="entry-toggle" onClick={() => goalMove(g.id, 1)} disabled={idx === plans.goalTargets.length - 1}>下移</button>
                      <button type="button" className="entry-toggle" onClick={() => goalCopy(g.id)}>复制</button>
                    </div>
                  </>
                )}
              </li>
